...
{field:a,recipients:b}))};this.check_compose_input=function(a){var b,d=$("[name='_to']"),e=$("[name='_cc']"),g=$("[name='_bcc']"),f=$("[name='_from']"),h=$("[name='_subject']"),k=$("[name='_message']");if("text"==f.prop("type")&&!rcube_check_email(f.val(),!0))return alert(this.get_label("nosenderwarning")),f.focus(),!1;e=d.val()?d.val():e.val()?e.val():g.val();if(!rcube_check_email(e.replace(/^\s+/,"").replace(/[\s,;]+$/,""),!0))return alert(this.get_label("norecipientwarning")),d.focus(),!1;for(var m in this.env.attachments)if("object"===
typeof this.env.attachments[m]&&!this.env.attachments[m].complete)return alert(this.get_label("notuploadedwarning")),!1;if(""==h.val()){b=$('<div class="prompt">').html('<div class="message">'+this.get_label("nosubjectwarning")+"</div>").appendTo(document.body);var l=$("<input>").attr("type","text").attr("size",30).appendTo(b).val(this.get_label("nosubject")),d={};d[this.get_label("cancel")]=function(){h.focus();$(this).dialog("close")};d[this.get_label("sendmessage")]=function(){h.val(l.val());$(this).dialog("close");
j.command(a,{nocheck:!0})};b.dialog({modal:!0,resizable:!1,buttons:d,close:function(){$(this).remove()}});l.select();return!1}this.stop_spellchecking();window.tinyMCE&&(b=tinyMCE.get(this.env.composebody));if(!b&&""==k.val()&&!confirm(this.get_label("nobodywarning")))return k.focus(),!1;if(b){if(!b.getContent()&&!confirm(this.get_label("nobodywarning")))return b.focus(),!1;tinyMCE.triggerSave()}return!0};this.toggle_editor=function(a){this.stop_spellchecking();"html"==a.mode?(this.plain2html($("#"+
a.id).val(),a.id),tinyMCE.execCommand("mceAddControl",!1,a.id),this.env.default_font&&setTimeout(function(){$(tinyMCE.get(a.id).getBody()).css("font-family",rcmail.env.default_font)},500)):this.html2plain(tinyMCE.get(a.id).getContent(),a.id)&&tinyMCE.execCommand("mceRemoveControl",!1,a.id);return!0};this.insert_response=function(a){a=this.env.textresponses[a]?this.env.textresponses[a].text:null;if(!a)return!1;if("1"==$("input[name='_is_html']").val()){var b=tinyMCE.get(this.env.composebody);b.getWin().focus();
b.selection.setContent(this.quote_html(a).replace(/\r?\n/g,"<br/>"),{format:"text"})}else{var b=rcube_find_object(this.env.composebody),d=$(b).is(":focus")?this.get_input_selection(b):{start:0,end:0},e=b.value;pre=e.substring(0,d.start);end=e.substring(d.end,e.length);b.value=pre+a+end;this.set_caret_pos(b,d.start+a.length);b.focus()}};this.save_response=function(){var a,b="";a=!1;if("1"==$("input[name='_is_html']").val()){var d=tinyMCE.get(this.env.composebody);d.getWin().focus();b=d.selection.getContent({format:"text"});
b||(b=d.getContent({format:"text"}),a=!0)}else{if((d=rcube_find_object(this.env.composebody))&&$(d).is(":focus"))b=this.get_input_selection(d).text;!b&&d&&(b=d.value,a=!0)}a&&(a=b.indexOf("-- \n"),0<a&&(b=b.substring(0,a)));a={};d='<form class="propform"><div class="prop block"><label>'+this.get_label("responsename")+'</label><input type="text" name="name" id="ffresponsename" size="40" /></div><div class="prop block"><label>'+this.get_label("responsetext")+'</label><textarea name="text" id="ffresponsetext" cols="40" rows="8"></textarea></div></form>';
a[this.gettext("save")]=function(){var a=$("#ffresponsename").val(),b=$("#ffresponsetext").val();if(!b)return $("#ffresponsetext").select(),!1;a||(a=b.substring(0,40));var d=j.display_message(j.get_label("savingresponse"),"loading");j.http_post("settings/responses",{_insert:1,_name:a,_text:b},d);$(this).dialog("close")};a[this.gettext("cancel")]=function(){$(this).dialog("close")};this.show_popup_dialog(d,this.gettext("savenewresponse"),a);$("#ffresponsetext").val(b);$("#ffresponsename").select()};
this.add_response_item=function(a){var b=a.key;this.env.textresponses[b]=a;if(this.gui_objects.responseslist){var d=$("<li>").appendTo(this.gui_objects.responseslist);$("<a>").addClass("insertresponse active").attr("href","#").attr("rel",b).html(this.quote_html(a.name)).appendTo(d).mousedown(function(a){return rcube_event.cancel(a)}).mouseup(function(a){j.command("insert-response",b);$(document.body).trigger("mouseup");return rcube_event.cancel(a)})}};this.edit_responses=function(){};this.delete_response=
...
function(a){var b;a&&a!=this.env.draft_id&&((b=this.opener())&&"mail"==b.env.task&&(""==b.env.action&&b.env.mailbox==this.env.drafts_mailbox)&&b.command("checkmail"),this.env.draft_id=a,$("input[name='_draft_saveid']").val(a),window.frames.savetarget&&(window.frames.savetarget.history&&!this.draft_autosave_submit)&&window.frames.savetarget.history.back(),this.draft_autosave_submit=!1);this.remove_compose_data(this.env.compose_id);this.compose_skip_unsavedcheck=!1};this.auto_save_start=function(){this.env.draft_autosave&&
(this.draft_autosave_submit=!1,this.save_timer=setTimeout(function(){j.draft_autosave_submit=!0;j.command("savedraft")},1E3*this.env.draft_autosave));!this.local_save_timer&&window.localStorage&&(this.compose_type_activity=this.compose_type_activity_last=0,$(document).bind("keypress",function(){j.compose_type_activity++}),this.local_save_timer=setInterval(function(){j.compose_type_activity>j.compose_type_activity_last&&(j.save_compose_form_local(),j.compose_type_activity_last=j.compose_type_activity)},
5E3),$(window).unload(function(){j.env.server_error||j.remove_compose_data(j.env.compose_id)}));window.onbeforeunload||(window.onbeforeunload=function(){if(!j.compose_skip_unsavedcheck&&j.cmp_hash!=j.compose_field_hash())return j.get_label("notsentwarning")});this.busy=!1};this.compose_field_hash=function(a){var b,d,e,g="",f=["to","cc","bcc","subject"];for(d=0;d<f.length;d++)if(e=$('[name="_'+f[d]+'"]').val())g+=e+":";g=window.tinyMCE&&(b=tinyMCE.get(this.env.composebody))?g+b.getContent():g+$("[name='_message']").val();
if(this.env.attachments)for(var h in this.env.attachments)g+=h;a&&(this.cmp_hash=g);return g};this.save_compose_form_local=function(){var a={session:this.env.session_id,changed:(new Date).getTime()},b=!0;window.tinyMCE&&tinyMCE.get(this.env.composebody)&&tinyMCE.triggerSave();this.env.draft_id&&(a.draft_id=this.env.draft_id);this.env.reply_msgid&&(a.reply_msgid=this.env.reply_msgid);$("input, select, textarea",this.gui_objects.messageform).each(function(d,e){switch(e.tagName.toLowerCase()){case "input":if("button"==
...
function(a,b){var d=this.local_storage_get_item("compose."+a,!0);d&&"object"==typeof d&&($.each(d,function(a,b){if("_"==a[0]){var d=$("*[name='"+a+"']");d[0]&&"checkbox"==d[0].type?d.prop("checked",""!=b):d.val(b)}}),"1"==d._is_html?b||(tinyMCE.execCommand("mceAddControl",!1,this.env.composebody),this.triggerEvent("aftertoggle-editor",{mode:"html"})):b&&(tinyMCE.execCommand("mceRemoveControl",!1,this.env.composebody),this.triggerEvent("aftertoggle-editor",{mode:"plain"})))};this.remove_compose_data=
...
var d=a.options[a.selectedIndex].value;this.env.signatures&&this.env.signatures[d]?(this.enable_command("insert-sig",!0),this.env.compose_commands.push("insert-sig")):this.enable_command("insert-sig",!1);if(!this.env.identities_initialized&&(this.env.identities_initialized=!0,this.env.show_sig_later&&(this.env.show_sig=!0),this.env.opened_extwin))return;var e;e=-1;var g=$("[name='_message']"),f=g.val(),h="1"==$("input[name='_is_html']").val(),k=this.env.identity,m=this.env.recipients_separator,l=
RegExp.escape(m);$.each(["replyto","bcc"],function(){var a,b=k&&j.env.identities[k]?j.env.identities[k][this]:"",e=d&&j.env.identities[d]?j.env.identities[d][this]:"",g=$('[name="_'+this+'"]'),f=g.val();b&&f&&(a=RegExp("\\s*"+RegExp.escape(b)+"\\s*"),f=f.replace(a,""));a=RegExp(l+"\\s*"+l,"g");f=f.replace(a,m);a=RegExp("^[\\s"+l+"]+");f=f.replace(a,"");e&&(-1==f.indexOf(e)&&-1==f.indexOf(e.replace(/"/g,"")))&&(f&&(a=RegExp("["+l+"\\s]+$"),f=f.replace(a,"")+m+" "),f+=e+m+" ");(b||e)&&g.val(f).change()});
h?b&&this.env.signatures&&(h=tinyMCE.get(this.env.composebody),g=h.dom.get("_rc_sig"),g||(f=h.getBody(),e=h.getDoc(),g=e.createElement("div"),g.setAttribute("id","_rc_sig"),this.env.top_posting?(h.getWin().focus(),h=h.selection.getNode(),"BODY"==h.nodeName?(f.insertBefore(g,f.firstChild),f.insertBefore(e.createElement("br"),f.firstChild)):(f.insertBefore(g,h.nextSibling),f.insertBefore(e.createElement("br"),h.nextSibling))):(bw.ie&&f.appendChild(e.createElement("br")),f.appendChild(g))),this.env.signatures[d]&&
(g.innerHTML=this.env.signatures[d].html)):(b&&(k&&this.env.signatures&&this.env.signatures[k])&&(k=this.env.signatures[k].text,k=k.replace(/\r\n/g,"\n"),e=this.env.top_posting?f.indexOf(k):f.lastIndexOf(k),0<=e&&(f=f.substring(0,e)+f.substring(e+k.length,f.length))),b&&this.env.signatures&&this.env.signatures[d]?(k=this.env.signatures[d].text,k=k.replace(/\r\n/g,"\n"),this.env.top_posting?0<=e?(f=f.substring(0,e)+k+f.substring(e,f.length),e-=1):f?(pos=this.get_caret_pos(g.get(0)))?(f=f.substring(0,
pos)+"\n"+k+"\n\n"+f.substring(pos,f.length),e=pos):(e=0,f="\n\n"+k+"\n\n"+f.replace(/^[\r\n]+/,"")):(e=0,f="\n\n"+k):(f=f.replace(/[\r\n]+$/,""),e=!this.env.top_posting&&f.length?f.length+1:0,f+="\n\n"+k)):e=this.env.top_posting?0:f.length,g.val(f),this.set_caret_pos(g.get(0),e));this.env.identity=d;this.triggerEvent("change_identity");return!0};this.upload_file=function(a,b){if(a){var d=0,e=0;$("input[type=file]",a).each(function(a,b){var f=b.files?b.files.length:b.value?1:0;if(b.files)for(a=0;a<
f;a++)d+=b.files[a].size;e+=f});if(e){if(this.env.max_filesize&&this.env.filesizeerror&&d>this.env.max_filesize)return this.display_message(this.env.filesizeerror,"error"),!1;var g=this.async_upload_form(a,b||"upload",function(a){var b,d="";try{this.contentDocument?b=this.contentDocument:this.contentWindow&&(b=this.contentWindow.document),d=b.childNodes[0].innerHTML}catch(e){}if(!d.match(/add2attachment/)&&(!bw.opera||rcmail.env.uploadframe&&rcmail.env.uploadframe==a.data.ts))d.match(/display_message/)||
rcmail.display_message(rcmail.get_label("fileuploaderror"),"error"),rcmail.remove_from_attachment_list(a.data.ts);bw.opera&&(rcmail.env.uploadframe=a.data.ts)}),f="<span>"+this.get_label("uploading"+(1<e?"many":""))+"</span>",h=g.replace(/^rcmupload/,"");this.add2attachment_list(h,{name:"",html:f,classname:"uploading",frame:g,complete:!1});this.env.upload_progress_time&&this.upload_progress_start("upload",h);this.gui_objects.attachmentform=a;return!0}}};this.add2attachment_list=function(a,b,d){if(!this.gui_objects.attachmentlist)return!1;
!b.complete&&j.env.loadingicon&&(b.html='<img src="'+j.env.loadingicon+'" alt="" class="uploading" />'+b.html);!b.complete&&b.frame&&(b.html='<a title="'+this.get_label("cancel")+'" onclick="return rcmail.cancel_attachment_upload(\''+a+"', '"+b.frame+'\');" href="#cancelupload" class="cancelupload">'+(this.env.cancelicon?'<img src="'+this.env.cancelicon+'" alt="" />':this.get_label("cancel"))+"</a>"+b.html);var e,g=$("<li>");g.attr("id",a).addClass(b.classname).html(b.html).on("mouseover",function(){rcube_webmail.long_subject_title_ex(this)});
d&&(e=document.getElementById(d))?g.replaceAll(e):g.appendTo(this.gui_objects.attachmentlist);d&&this.env.attachments[d]&&delete this.env.attachments[d];this.env.attachments[a]=b;return!0};this.remove_from_attachment_list=function(a){this.env.attachments&&(delete this.env.attachments[a],$("#"+a).remove())};this.remove_attachment=function(a){a&&this.env.attachments[a]&&this.http_post("remove-attachment",{_id:this.env.compose_id,_file:a});return!0};this.cancel_attachment_upload=function(a,b){if(!a||
...
return;case 37:case 39:return}this.ksearch_timer=setTimeout(function(){j.ksearch_get_results(d)},200);this.ksearch_input=b;return!0};this.ksearch_visible=function(){return null!==this.ksearch_selected&&void 0!==this.ksearch_selected&&this.ksearch_value};this.ksearch_select=function(a){var b=$("#rcmksearchSelected");b[0]&&a&&b.removeAttr("id").removeClass("selected");a&&($(a).attr("id","rcmksearchSelected").addClass("selected"),this.ksearch_selected=a._rcm_id)};this.insert_recipient=function(a){if(!(null===
a||!this.env.contacts[a]||!this.ksearch_input)){var b=this.ksearch_input.value,d=this.get_caret_pos(this.ksearch_input),d=b.lastIndexOf(this.ksearch_value,d),e=!1,g="",f=b.substring(0,d),b=b.substring(d+this.ksearch_value.length,b.length);this.ksearch_destroy();"object"===typeof this.env.contacts[a]&&this.env.contacts[a].id?(g+=this.env.contacts[a].name+this.env.recipients_delimiter,this.group2expand[this.env.contacts[a].id]=$.extend({input:this.ksearch_input},this.env.contacts[a]),this.http_request("mail/group-expand",
{_source:this.env.contacts[a].source,_gid:this.env.contacts[a].id},!1)):"string"===typeof this.env.contacts[a]&&(g=this.env.contacts[a]+this.env.recipients_delimiter,e=!0);this.ksearch_input.value=f+g+b;this.set_caret_pos(this.ksearch_input,d+g.length);e&&(this.triggerEvent("autocomplete_insert",{field:this.ksearch_input,insert:g}),this.compose_type_activity++)}};this.replace_group_recipients=function(a,b){this.group2expand[a]&&(this.group2expand[a].input.value=this.group2expand[a].input.value.replace(this.group2expand[a].name,
b),this.triggerEvent("autocomplete_insert",{field:this.group2expand[a].input,insert:b}),this.group2expand[a]=null,this.compose_type_activity++)};this.ksearch_get_results=function(a){var b=this.ksearch_input?this.ksearch_input.value:null;if(null!==b){this.ksearch_pane&&this.ksearch_pane.is(":visible")&&this.ksearch_pane.hide();var d=this.get_caret_pos(this.ksearch_input),e=b.lastIndexOf(this.env.recipients_separator,d-1),b=b.substring(e+1,d),e=this.env.autocomplete_min_length,d=this.ksearch_data,b=
$.trim(b);if(b!=this.ksearch_value)if(this.ksearch_destroy(),b.length&&b.length<e)this.ksearch_info||(this.ksearch_info=this.display_message(this.get_label("autocompletechars").replace("$min",e)));else if(e=this.ksearch_value,this.ksearch_value=b,b.length&&(!e||!e.length||!b.startsWith(e)||d&&!(0>=d.num)||!this.env.contacts||this.env.contacts.length)){var g,f,e=(new Date).getTime(),b={_search:b,_id:e},d=a&&a.threads?a.threads:1;g=a&&a.sources?a.sources:[];a=a&&a.action?a.action:"mail/autocomplete";
this.ksearch_data={id:e,sources:g.slice(),action:a,locks:[],requests:[],num:g.length};for(e=0;e<d;e++){g=this.ksearch_data.sources.shift();if(1<d&&void 0===g)break;b._source=g?g:"";g=this.display_message(this.get_label("searching"),"loading");f=this.http_post(a,b,g);this.ksearch_data.locks.push(g);this.ksearch_data.requests.push(f)}}}};this.ksearch_query_results=function(a,b,d){if(this.ksearch_value&&!(this.ksearch_input&&b!=this.ksearch_value)){var e,g,f,h,k,m=this.ksearch_value;b=this.ksearch_data;
var l=this.env.autocomplete_max?this.env.autocomplete_max:15;this.ksearch_pane||(f=$("<ul>"),this.ksearch_pane=$("<div>").attr("id","rcmKSearchpane").css({position:"absolute","z-index":3E4}).append(f).appendTo(document.body),this.ksearch_pane.__ul=f[0]);f=this.ksearch_pane.__ul;d&&this.ksearch_pane.data("reqid")==d?l-=f.childNodes.length:(this.ksearch_pane.data("reqid",d),f.innerHTML="",this.env.contacts=[],e=$(this.ksearch_input).offset(),this.ksearch_pane.css({left:e.left+"px",top:e.top+this.ksearch_input.offsetHeight+
"px",display:"none"}));if(a&&(g=a.length))for(e=0;e<g&&0<l;e++)k="object"===typeof a[e]?a[e].name:a[e],h=document.createElement("LI"),h.innerHTML=k.replace(RegExp("("+RegExp.escape(m)+")","ig"),"##$1%%").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/##([^%]+)%%/g,"<b>$1</b>"),h.onmouseover=function(){j.ksearch_select(this)},h.onmouseup=function(){j.ksearch_click(this)},h._rcm_id=this.env.contacts.length+e,f.appendChild(h),l-=1;f.childNodes.length&&(this.ksearch_pane.show(),this.env.contacts.length||
($("li:first",f).attr("id","rcmksearchSelected").addClass("selected"),this.ksearch_selected=0));g&&(this.env.contacts=this.env.contacts.concat(a));if(b.id==d)if(b.num--,0<l&&b.sources.length){if(a=b.sources.shift())a={_search:m,_id:d,_source:a},d=this.display_message(this.get_label("searching"),"loading"),b=this.http_post(b.action,a,d),this.ksearch_data.locks.push(d),this.ksearch_data.requests.push(b)}else l||(this.ksearch_msg||(this.ksearch_msg=this.display_message(this.get_label("autocompletemore"))),
...
this.hide_message(this.ksearch_info);this.ksearch_msg&&this.hide_message(this.ksearch_msg);this.ksearch_msg=this.ksearch_info=this.ksearch_data=null};this.ksearch_abort=function(){var a,b,d=this.ksearch_data;if(d){a=0;for(b=d.locks.length;a<b;a++)this.abort_request({request:d.requests[a],lock:d.locks[a]})}};this.contactlist_keypress=function(a){a.key_pressed==a.DELETE_KEY&&this.command("delete")};this.contactlist_select=function(a){this.preview_timer&&clearTimeout(this.preview_timer);var b,d,e,g,
f=this,h=!1,k=this.env.source?this.env.address_sources[this.env.source]:null;(d=a.get_single_selection())?this.preview_timer=setTimeout(function(){f.load_contact(d,"show")},200):this.env.contentframe&&this.show_contentframe(!1);if(a.selection.length){a.draggable=!1;this.env.selection_sources=[];k&&this.env.selection_sources.push(this.env.source);for(b in a.selection){g=a.data[a.selection[b]];if(k)h=h||!k.readonly&&!g.readonly;else if((e=String(a.selection[b]).replace(/^[^-]+-/,""))&&this.env.address_sources[e])h=
h||!this.env.address_sources[e].readonly&&!g.readonly,this.env.selection_sources.push(e);"group"!=g._type&&(a.draggable=!0)}this.env.selection_sources=$.unique(this.env.selection_sources)}this.enable_command("group-remove-selected",this.env.group&&0<a.selection.length&&h);this.enable_command("compose",this.env.group||0<a.selection.length);this.enable_command("export-selected","copy",0<a.selection.length);this.enable_command("edit",d&&h);this.enable_command("delete","move",0<a.selection.length&&h);
return!1};this.list_contacts=function(a,b,d){var e,g={},f=void 0===a&&void 0===b&&void 0===d,h=window;a||(a=this.env.source);if(d&&this.current_page==d&&a==this.env.source&&b==this.env.group)return!1;a!=this.env.source?(d=this.env.current_page=1,this.reset_qsearch()):!f&&b!=this.env.group&&(d=this.env.current_page=1);this.env.search_id?e="S"+this.env.search_id:this.env.search_request||(e=b?"G"+a+b:a);this.env.source=a;this.env.group=b;f=$.inArray(this.env.group,this.env.address_group_stack);this.env.address_group_stack=
0>f?[]:this.env.address_group_stack.slice(0,f);this.env.group?(this.env.address_group_stack.push(this.env.group),e="G"+a+this.env.address_group_stack[0]):this.gui_objects.addresslist_title&&$(this.gui_objects.addresslist_title).html(this.get_label("contacts"));this.select_folder(e,"",!0);if(this.gui_objects.contactslist)this.list_contacts_remote(a,b,d);else{if(e=this.get_frame_window(this.env.contentframe))h=e,g._framed=1;b&&(g._gid=b);d&&(g._page=d);a&&(g._source=a);this.env.search_request&&(g._search=
this.env.search_request);this.set_busy(!0,"loading");this.location_href(g,h)}};this.list_contacts_remote=function(a,b,d){this.list_contacts_clear();var e={},g=this.set_busy(!0,"loading");a&&(e._source=a);d&&(e._page=d);b&&(e._gid=b);this.env.source=a;this.env.group=b;this.env.search_request&&(e._search=this.env.search_request);this.http_request("mail"==this.env.task?"list-contacts":"list",e,g)};this.list_contacts_clear=function(){this.contact_list.data={};this.contact_list.clear(!0);this.show_contentframe(!1);
this.enable_command("delete","move","copy",!1);this.enable_command("compose",this.env.group?!0:!1)};this.set_group_prop=function(a){if(this.gui_objects.addresslist_title){var b=$(this.gui_objects.addresslist_title).html("");1<this.env.address_group_stack.length&&($('<a href="#list">...</a>').addClass("poplink").appendTo(b).click(function(){return j.command("popgroup","",this)}),b.append("&nbsp;&raquo;&nbsp;"));b.append($("<span>").text(a.name))}this.triggerEvent("groupupdate",a)};this.load_contact=
function(a,b,d){var e,g={},f=window,h=this.contact_list?this.contact_list.data[a]:null;if(e=this.get_frame_window(this.env.contentframe))g._framed=1,f=e,this.show_contentframe(!0),a||this.contact_list.clear_selection(),this.enable_command("compose",h&&h.email),this.enable_command("export-selected",h&&"group"!=h._type);else if(d)return!1;if(b&&(a||"add"==b)&&!this.drag_active)this.env.group&&(g._gid=this.env.group),this.env.search_request&&(g._search=this.env.search_request),g._action=b,g._source=
...
d.innerHTML=b[g],h.cols.push(d);f.data[a]=e;f.insert_row(h);this.enable_command("export",0<f.rowcount)};this.init_contact_form=function(){var a=this,b;if(this.env.coltypes)for(b in this.set_photo_actions($("#ff_photo").val()),this.env.coltypes)this.init_edit_field(b,null);$(".contactfieldgroup .row a.deletebutton").click(function(){a.delete_edit_field(this);return!1});$("select.addfieldmenu").change(function(){a.insert_edit_field($(this).val(),$(this).attr("rel"),this);this.selectedIndex=0});$.datepicker&&
this.env.date_format&&($.datepicker.setDefaults({dateFormat:this.env.date_format,changeMonth:!0,changeYear:!0,yearRange:"-100:+10",showOtherMonths:!0,selectOtherMonths:!0,onSelect:function(a){$(this).focus().val(a)}}),$("input.datepicker").datepicker());"search"==this.env.action&&$(this.gui_objects.editform).append($('<input type="submit">').hide()).submit(function(){$("input.mainaction").click();return!1})};this.group_create=function(){this.add_input_row("contactgroup")};this.group_rename=function(){if(this.env.group&&
...
parent.rcmail.replace_folder_row(a,b,d,e,g):!1;var f,h,k,j,l=this.get_folder_row_id(a),n=a.length;f=$('input[name="_subscribed[]"]',$("#"+l)).prop("checked");var p=this.get_subfolders(a);if(a==b)$("#"+l).attr("class",g||""),this.subscription_list.focus();else{this._remove_folder_row(l);e=$(this.add_folder_row(b,d,e,f,!0,g));if(d=p.length)j=a.split(this.env.delimiter).length-b.split(this.env.delimiter).length;for(a=0;a<d;a++)if(l=p[a],f=this.env.subscriptionrows[l][0],g=this.env.subscriptionrows[l][1],
h=$("#"+l),k=h.clone(!0),h.remove(),e.after(k),e=k,f=b+f.slice(n),$('input[name="_subscribed[]"]',e).val(f),this.env.subscriptionrows[l][0]=f,0!=j){if(0<j)for(f=j;0<f;f--)g=g.replace(/^&nbsp;&nbsp;&nbsp;&nbsp;/,"");else for(f=j;0>f;f++)g="&nbsp;&nbsp;&nbsp;&nbsp;"+g;e.find("td:first").html(g);this.env.subscriptionrows[l][1]=g}this.init_subscription_list()}};this.remove_folder_row=function(a,b){var d,e,g=[];d=this.get_folder_row_id(a);b&&(g=this.get_subfolders(a));this._remove_folder_row(d);d=0;for(e=
