...
13==rcube_event.get_keycode(a))return g.command("insert-response",$(this).attr("rel")),$(document.body).trigger("mouseup"),rcube_event.cancel(a)}),$.each(this.buttons["save-response"]||[],function(a,b){$("#"+b.id).mousedown(function(a){return rcube_event.cancel(a)})})),this.init_messageform()):"get"==this.env.action?this.enable_command("download","print",!0):"print"==this.env.action&&this.env.uid&&(bw.safari?setTimeout("window.print()",10):window.print());this.gui_objects.mailboxlist&&(this.env.unread_counts=
{},this.gui_objects.folderlist=this.gui_objects.mailboxlist,this.http_request("getunread"));this.gui_objects.contactslist&&(this.contact_list=new rcube_list_widget(this.gui_objects.contactslist,{multiselect:!0,draggable:!1,keyboard:!0}),this.contact_list.addEventListener("initrow",function(a){g.triggerEvent("insertrow",{cid:a.uid,row:a})}).addEventListener("select",function(a){g.compose_recipient_select(a)}).addEventListener("dblclick",function(a){g.compose_add_recipient()}).addEventListener("keypress",
...
this.env.cid&&(this.enable_command("show","edit",!0),this.gui_objects.editform&&$("input.groupmember").change(function(){g.group_member_change(this.checked?"add":"del",g.env.cid,g.env.source,this.value)}));this.gui_objects.editform&&(this.enable_command("save",!0),"add"!=this.env.action&&"edit"!=this.env.action&&"search"!=this.env.action||this.init_contact_form());break;case "settings":this.enable_command("preferences","identities","responses","save","folders",!0);"identities"==this.env.action?this.enable_command("add",
2>this.env.identities_level):"edit-identity"==this.env.action||"add-identity"==this.env.action?(this.enable_command("save","edit","toggle-editor",!0),this.enable_command("delete",2>this.env.identities_level),this.editor_init(this.env.editor_config,"rcmfd_signature")):"folders"==this.env.action?this.enable_command("subscribe","unsubscribe","create-folder","rename-folder",!0):"edit-folder"==this.env.action&&this.gui_objects.editform?(this.enable_command("save","folder-size",!0),parent.rcmail.env.exists=
this.env.messagecount,parent.rcmail.enable_command("purge",this.env.messagecount)):"responses"==this.env.action&&this.enable_command("add",!0);this.gui_objects.identitieslist?(this.identity_list=new rcube_list_widget(this.gui_objects.identitieslist,{multiselect:!1,draggable:!1,keyboard:!0}),this.identity_list.addEventListener("select",function(a){g.identity_select(a)}).addEventListener("keypress",function(a){a.key_pressed==a.ENTER_KEY&&g.identity_select(a)}).init().focus()):this.gui_objects.sectionslist?
(this.sections_list=new rcube_list_widget(this.gui_objects.sectionslist,{multiselect:!1,draggable:!1,keyboard:!0}),this.sections_list.addEventListener("select",function(a){g.section_select(a)}).addEventListener("keypress",function(a){a.key_pressed==a.ENTER_KEY&&g.section_select(a)}).init().focus()):this.gui_objects.subscriptionlist?this.init_subscription_list():this.gui_objects.responseslist&&(this.responses_list=new rcube_list_widget(this.gui_objects.responseslist,{multiselect:!1,draggable:!1,keyboard:!0}),
this.responses_list.addEventListener("select",function(a){var b;a=a.get_single_selection();g.enable_command("delete",!!a&&0>$.inArray(a,g.env.readonly_responses));a&&(b=g.get_frame_window(g.env.contentframe))&&(g.set_busy(!0),g.location_href({_action:"edit-response",_key:a,_framed:1},b))}).init().focus());break;case "login":d=$("#rcmloginuser"),d.bind("keyup",function(a){return g.login_user_keyup(a)}),""==d.val()?d.focus():$("#rcmloginpwd").focus(),window.jstz?(d=jstz.determine(),d.name()&&$("#rcmlogintz").val(d.name())):
$("#rcmlogintz").val((new Date).getStdTimezoneOffset()/-60),$("form").submit(function(){$("input[type=submit]",this).prop("disabled",!0);g.clear_messages();g.display_message("","loading")}),this.enable_command("login",!0)}this.gui_objects.editform&&$("input,select,textarea",this.gui_objects.editform).not(":hidden").not(":disabled").first().select().focus();this.env.contentframe&&!$("#"+this.env.contentframe).is(":visible")&&(this.env.contentframe=null);bw.ie&&$("input[type=file]").keydown(function(a){"13"==
...
function(a){g.folder_collapsed(a)}).addEventListener("expand",function(a){g.folder_collapsed(a)}).addEventListener("beforeselect",function(a){return!g.busy}).addEventListener("select",function(a){g.triggerEvent("selectfolder",{folder:a.id,prefix:"rcmli"})}));this.gui_objects.filedrop&&this.env.filedrop&&(window.XMLHttpRequest&&XMLHttpRequest.prototype&&XMLHttpRequest.prototype.sendAsBinary||window.FormData)&&($(document.body).bind("dragover dragleave drop",function(a){return g.document_drag_hover(a,
"dragover"==a.type)}),$(this.gui_objects.filedrop).addClass("droptarget").bind("dragover dragleave",function(a){return g.file_drag_hover(a,"dragover"==a.type)}).get(0).addEventListener("drop",function(a){return g.file_dropped(a)},!1));var f=function(a){return g.doc_mouse_up(a)};$(document.body).bind("mouseup",f).bind("keydown",function(a){return g.doc_keypress(a)});$("iframe").load(function(a){try{$(this.contentDocument||this.contentWindow).on("mouseup",f)}catch(b){}}).contents().on("mouseup",f);
this.triggerEvent("init",{task:this.task,action:this.env.action});for(a in this.onloads)if("string"===typeof this.onloads[a])eval(this.onloads[a]);else if("function"===typeof this.onloads[a])this.onloads[a]();this.start_refresh();this.start_keepalive()}};this.log=function(a){window.console&&console.log&&console.log(a)};this.command=function(a,b,d,e){var f,h,n,k=!1;!d||!d.blur||e&&rcube_event.is_keyboard(e)||d.blur();if(this.busy&&("reset-search"!=a||"search"!=this.last_command)&&!a.match(/^menu-/))return!1;
if(d&&d.href&&0>String(d.href).indexOf("#")&&rcube_event.get_modifier(e))return!0;if(!this.commands[a])return this.is_framed()&&parent.rcmail.command(a,b),!1;if("mail"==this.task&&"compose"==this.env.action&&0>$.inArray(a,this.env.compose_commands)&&!this.env.server_error){if(this.cmp_hash!=this.compose_field_hash()&&!confirm(this.get_label("notsentwarning")))return!1;this.remove_compose_data(this.env.compose_id);this.compose_skip_unsavedcheck=!0}this.last_command=a;if("function"===typeof this.command_handlers[a])return f=
this.command_handlers[a](b,d,e),void 0!==f?f:d?!1:!0;if("string"===typeof this.command_handlers[a])return f=window[this.command_handlers[a]](b,d,e),void 0!==f?f:d?!1:!0;this.triggerEvent("actionbefore",{props:b,action:a,originalEvent:e});f=this.triggerEvent("before"+a,b||e);if(void 0!==f){if(!1===f)return!1;b=f}f=void 0;switch(a){case "login":this.gui_objects.loginform&&this.gui_objects.loginform.submit();break;case "logout":case "mail":case "addressbook":case "settings":this.switch_task(a);break;
case "about":this.redirect("?_task=settings&_action=about",!1);break;case "permaurl":if(d&&d.href&&d.target)return!0;this.env.permaurl&&(parent.location.href=this.env.permaurl);break;case "extwin":if("compose"==this.env.action){if(e=this.gui_objects.messageform,h=this.open_window(""))this.save_compose_form_local(),this.compose_skip_unsavedcheck=!0,$("input[name='_action']",e).val("compose"),e.action=this.url("mail/compose",{_id:this.env.compose_id,_extwin:1}),e.target=h.name,e.submit()}else this.open_window(this.env.permaurl,
...
case "print":if("get"==this.env.action)this.gui_objects.messagepartframe.contentWindow.print();else if(h=this.get_single_uid())e="&_action=print&_uid="+h+"&_mbox="+urlencode(this.get_message_mailbox(h))+(this.env.safemode?"&_safe=1":""),this.open_window(this.env.comm_path+e,!0,!0)&&"show"!=this.env.action&&this.mark_message("read",h);break;case "viewsource":(h=this.get_single_uid())&&this.open_window(this.env.comm_path+"&_action=viewsource&_uid="+h+"&_mbox="+urlencode(this.env.mailbox),!0,!0);break;
case "download":"get"==this.env.action?location.href=location.href.replace(/_frame=/,"_download="):(h=this.get_single_uid())&&this.goto_url("viewsource",{_uid:h,_mbox:this.get_message_mailbox(h),_save:1});break;case "search":if(!b&&this.gui_objects.qsearchbox&&(b=this.gui_objects.qsearchbox.value),b){this.qsearch(b);break}case "reset-search":var l;e=this.env.search_request||this.env.qsearch;h=this.gui_objects.qsearchbox&&""!=this.gui_objects.qsearchbox.value;this.reset_qsearch();this.select_all_mode=
!1;if(e&&"compose"==this.env.action)this.contact_list&&this.list_contacts_clear();else if(e&&h&&this.env.mailbox)this.list_mailbox(this.env.mailbox,1);else if(e&&"addressbook"==this.task){if(""==this.env.source){for(l in this.env.address_sources)break;this.env.source=l;this.env.group=""}this.list_contacts(this.env.source,this.env.group,1)}break;case "pushgroup":this.env.address_group_stack.push(b.id),d&&e&&rcube_event.cancel(e);case "listgroup":this.reset_qsearch();this.list_contacts(b.source,b.id);
break;case "popgroup":1<this.env.address_group_stack.length&&(this.env.address_group_stack.pop(),this.reset_qsearch(),this.list_contacts(b.source,this.env.address_group_stack[this.env.address_group_stack.length-1]));break;case "import-messages":e=b||this.gui_objects.importform;h=this.set_busy(!0,"importwait");$('input[name="_unlock"]',e).val(h);(e=this.upload_file(e,"import"))||(this.set_busy(!1,null,h),!1!==e&&alert(this.get_label("selectimportfile")),k=!0);break;case "import":if("import"==this.env.action&&
...
this.treelist.toggle(a)};this.folder_collapsed=function(a){var b="addressbook"==this.env.task?"collapsed_abooks":"collapsed_folders";if(a.collapsed)this.env[b]=this.env[b]+"&"+urlencode(a.id)+"&",!a.virtual&&this.env.mailbox&&this.env.mailbox.startsWith(name+this.env.delimiter)&&this.command("list",name);else{var d=new RegExp("&"+urlencode(a.id)+"&");this.env[b]=this.env[b].replace(d,"")}this.drag_active||(this.command("save-pref",{name:b,value:this.env[b]}),this.env.unread_counts&&this.set_unread_count_display(a.id,
...
1;0<=k;k--)n=g.menu_stack[k],b=$("#"+n),!b.is(":visible")||d==b.data("opener")||d==b.get(0)||m.is(b.data("opener"))||n==h||"true"==b.attr("data-editable")&&$(d).parents("#"+n).length||"true"==b.attr("data-sticky")&&rcube_mouse_is_over(a,b.get(0))||g.hide_menu(n,a),h=b.data("parent")},10,a)}};this.doc_keypress=function(a){var b=function(a){var b,d=0>a?"prevAll":"nextAll",e=0>a?"last":"first";return g.focused_menu&&(b=$("#"+g.focused_menu))?(a=b.find(":focus").closest("li")[d](":has(:not([aria-disabled=true]))").find("a,input")[e](),
...
d.parent_uid:0,selected:this.select_all_mode||this.message_list.in_selection(a),ml:d.ml?1:0,ctype:d.ctype,mbox:d.mbox,flags:d.extra_flags});var f,h,g,k="",m="",l="",p="",q=this.message_list,w=q.rows;g=this.env.messages[a];var t=this.html_identifier(a,!0),u="message"+(d.seen?"":" unread")+(d.deleted?" deleted":"")+(d.flagged?" flagged":"")+(g.selected?" selected":""),x={cols:[],style:{},id:"rcmrow"+t,uid:a};f="msgicon";null===this.env.status_col&&(f+=" status",d.deleted?(k+=" deleted",m+=this.get_label("deleted")+
" "):d.seen?0<d.unread_children&&(k+=" unreadchildren"):(k+=" unread",m+=this.get_label("unread")+" "));d.answered&&(k+=" replied",m+=this.get_label("replied")+" ");d.forwarded&&(k+=" forwarded",m+=this.get_label("forwarded")+" ");g.selected&&!q.in_selection(a)&&q.selection.push(a);this.env.threading&&(g.depth?(l+='<span id="rcmtab'+t+'" class="branch" style="width:'+15*g.depth+'px;">&nbsp;&nbsp;</span>',w[g.parent_uid]&&!1===w[g.parent_uid].expanded||!(0!=this.env.autoexpand_threads&&2!=this.env.autoexpand_threads||
w[g.parent_uid]&&w[g.parent_uid].expanded)?(x.style.display="none",g.expanded=!1):g.expanded=!0,u+=" thread expanded"):g.has_children&&(void 0===g.expanded&&(1==this.env.autoexpand_threads||2==this.env.autoexpand_threads&&g.unread_children)&&(g.expanded=!0),p='<div id="rcmexpando'+x.id+'" class="'+(g.expanded?"expanded":"collapsed")+'">&nbsp;&nbsp;</div>',u+=" thread"+(g.expanded?" expanded":"")),d.unread_children&&d.seen&&!g.expanded&&(u+=" unroot"));l+='<span id="msgicn'+x.id+'" class="'+f+k+'" title="'+
m+'"></span>';x.className=u;b.subject&&(f=d.mbox==this.env.drafts_mailbox?"compose":"show",m={_mbox:d.mbox},m[d.mbox==this.env.drafts_mailbox?"_draft_uid":"_uid"]=a,b.subject='<a href="'+this.url(f,m)+'" onclick="return rcube_event.keyboard_only(event)" onmouseover="rcube_webmail.long_subject_title(this,'+(g.depth+1)+')" tabindex="-1"><span>'+b.subject+"</span></a>");for(h in this.env.listcols)f=this.env.listcols[h],a={className:String(f).toLowerCase(),events:{}},this.env.coltypes[f]&&this.env.coltypes[f].hidden&&
(a.className+=" hidden"),"flag"==f?(f=d.flagged?"flagged":"unflagged",g=this.get_label(f),f='<span id="flagicn'+x.id+'" class="'+f+'" title="'+g+'"></span>'):"attachment"==f?(g=this.get_label("withattachment"),f=d.attachmentClass?'<span class="'+d.attachmentClass+'" title="'+g+'"></span>':/application\/|multipart\/(m|signed)/.test(d.ctype)?'<span class="attachment" title="'+g+'"></span>':/multipart\/report/.test(d.ctype)?'<span class="report"></span>':"&nbsp;"):"status"==f?(g="",d.deleted?(f="deleted",
g=this.get_label("deleted")):d.seen?f=0<d.unread_children?"unreadchildren":"msgicon":(f="unread",g=this.get_label("unread")),f='<span id="statusicn'+x.id+'" class="'+f+k+'" title="'+g+'"></span>'):"threads"==f?f=p:"subject"==f?(bw.ie&&(a.events.mouseover=function(){rcube_webmail.long_subject_title_ex(this)}),f=l+b[f]):"priority"==f?0<d.prio&&6>d.prio?(g=this.get_label("priority")+" "+d.prio,f='<span class="prio'+d.prio+'" title="'+g+'"></span>'):f="&nbsp;":f="folder"==f?'<span onmouseover="rcube_webmail.long_subject_title(this)">'+
b[f]+"<span>":b[f],a.innerHTML=f,x.cols.push(a);q.insert_row(x,e);e&&this.env.pagesize&&q.rowcount>this.env.pagesize&&(a=q.get_last_row(),q.remove_row(a),q.clear_selection(a))};this.set_list_sorting=function(a,b){$("#rcm"+this.env.sort_col).removeClass("sorted"+this.env.sort_order.toUpperCase());a&&$("#rcm"+a).addClass("sorted"+b);this.env.sort_col=a;this.env.sort_order=b};this.set_list_options=function(a,b,d,e){var f,h={};void 0===b&&(b=this.env.sort_col);d||(d=this.env.sort_order);if(this.env.sort_col!=
b||this.env.sort_order!=d)f=1,this.set_list_sorting(b,d);this.env.threading!=e&&(f=1,h._threads=e);if(a&&a.length){var g,k,m=[],l=this.env.listcols;for(e=0;e<l.length;e++)k=l[e],g=$.inArray(k,a),-1!=g&&(m.push(k),delete a[g]);for(e=0;e<a.length;e++)a[e]&&m.push(a[e]);m.join()!=l.join()&&(f=1,h._cols=m.join(","))}f&&this.list_mailbox("","",b+"_"+d,h)};this.show_message=function(a,b,d){if(a){var e,f=window,h="&_action="+(d?"preview":"show")+"&_uid="+a+"&_mbox="+urlencode(this.get_message_mailbox(a));
d&&(e=this.get_frame_window(this.env.contentframe))&&(f=e,h+="&_framed=1");b&&(h+="&_safe=1");this.env.search_request&&(h+="&_search="+this.env.search_request);h+="&_caps="+urlencode(this.browser_capabilities());this.env.extwin&&(h+="&_extwin=1");d&&0<=String(f.location.href).indexOf(h)?this.show_contentframe(!0):(d||!this.env.message_extwin||this.env.extwin?this.location_href(this.env.comm_path+h,f,!0):this.open_window(this.env.comm_path+h,!0),d&&this.message_list&&this.message_list.rows[a]&&this.message_list.rows[a].unread&&
0<this.env.preview_pane_mark_read&&(this.preview_read_timer=setTimeout(function(){g.set_unread_message(a,g.env.mailbox);g.http_post("mark",{_uid:a,_flag:"read",_mbox:g.env.mailbox,_quiet:1})},1E3*this.env.preview_pane_mark_read)))}};this.set_unread_message=function(a,b){var d=this;d.message_list||(d=d.opener());!d&&window.parent&&(d=parent.rcmail);d&&d.message_list&&(!1===d.set_message(a,"unread",!1)&&d.set_message(a+"-"+b,"unread",!1),0<d.env.unread_counts[b]&&(--d.env.unread_counts[b],d.set_unread_count(b,
...
(this.env.current_page=a,"addressbook"==this.task||this.contact_list?this.list_contacts(this.env.source,this.env.group,a):"mail"==this.task&&this.list_mailbox(this.env.mailbox,a))};this.checkmail=function(){var a=this.set_busy(!0,"checkingmail"),b=this.check_recent_params();this.http_post("check-recent",b,a)};this.filter_mailbox=function(a){var b=this.set_busy(!0,"searching");this.clear_message_list();this.env.current_page=1;this.env.search_filter=a;this.http_request("search",this.search_params(!1,
a),b)};this.refresh_list=function(){this.list_mailbox(this.env.mailbox,this.env.current_page||1,null,{_clear:1},!0);this.message_list&&this.message_list.clear_selection()};this.list_mailbox=function(a,b,d,e,f){var h=window;"object"!=typeof e&&(e={});a||(a=this.env.mailbox?this.env.mailbox:"INBOX");d&&(e._sort=d);this.env.search_request&&(e._search=this.env.search_request);this.env.mailbox!=a&&(b=1,this.env.current_page=b,this.select_all_mode=!1);if(!f){this.clear_message_list();if(a!=this.env.mailbox||
a==this.env.mailbox&&!b&&!d)e._refresh=1;this.select_folder(a,"",!0);this.unmark_folder(a,"recent","",!0);this.env.mailbox=a}if(this.gui_objects.messagelist)this.list_mailbox_remote(a,b,e);else{if(d=this.get_frame_window(this.env.contentframe))h=d,e._framed=1;this.env.uid&&(e._uid=this.env.uid);a&&(this.set_busy(!0,"loading"),e._mbox=a,b&&(e._page=b),this.location_href(e,h))}};this.clear_message_list=function(){this.env.messages={};this.show_contentframe(!1);this.message_list&&this.message_list.clear(!0)};
this.list_mailbox_remote=function(a,b,d){var e=this.set_busy(!0,"loading");"object"!=typeof d&&(d={});d._mbox=a;b&&(d._page=b);this.http_request("list",d,e);this.update_state({_mbox:a,_page:b&&1<b?b:null})};this.update_selection=function(){var a=this.message_list.selection,b=this.message_list.rows,d,e=[];for(d in a)b[a[d]]&&e.push(a[d]);this.message_list.selection=e;try{var f=this.get_frame_window(this.env.contentframe).rcmail.env.uid;f&&0>$.inArray(f,e)&&this.show_contentframe(!1)}catch(h){}};this.expand_unread=
...
break;case 1:this.message_list.expand_all()}};this.init_threads=function(a,b){if(b&&b!=this.env.mailbox)return!1;for(var d=0,e=a.length;d<e;d++)this.add_tree_icons(a[d]);this.expand_threads()};this.add_tree_icons=function(a){var b,d,e,f,h=[],g=[],k,m=this.message_list.rows;for(k=a?m[a]?m[a].obj:null:this.message_list.tbody.firstChild;k;){if(1==k.nodeType&&(d=m[k.uid]))if(d.depth){for(b=h.length-1;0<=b&&!(e=h[b].length,e>d.depth?(f=e-d.depth,h[b][f]&2||(h[b][f]=h[b][f]?h[b][f]+2:2)):e==d.depth&&(h[b][0]&
2||(h[b][0]+=2)),d.depth>e);b--);h.push(Array(d.depth));h[h.length-1][0]=1;g.push(d.uid)}else{if(h.length){for(b in h)this.set_tree_icons(g[b],h[b]);h=[];g=[]}if(a&&k!=m[a].obj)break}k=k.nextSibling}if(h.length)for(b in h)this.set_tree_icons(g[b],h[b])};this.set_tree_icons=function(a,b){var d,e=[],f="",h=b.length;for(d=0;d<h;d++)2<b[d]?e.push({"class":"l3",width:15}):1<b[d]?e.push({"class":"l2",width:15}):0<b[d]?e.push({"class":"l1",width:15}):e.length&&!e[e.length-1]["class"]?e[e.length-1].width+=
...
this.selection_post_data();a._uid&&(this.show_contentframe(!1),this._with_selected_messages("delete",a))};this._with_selected_messages=function(a,b,d){var e=0,f="delete"==a||!this.is_multifolder_listing();if(this.message_list){var h,g,k,m=[],l=this.message_list.get_selection();h=0;for(len=l.length;h<len;h++)g=l[h],this.env.threading&&(e+=this.update_thread(g),k=this.message_list.find_root(g),k!=g&&0>$.inArray(k,m)&&m.push(k)),f&&this.message_list.remove_row(g,this.env.display_next&&h==l.length-1);
!this.env.display_next&&f&&this.message_list.clear_selection();h=0;for(len=m.length;h<len;h++)this.add_tree_icons(m[h])}0>e?b._count=-1*e:0<e&&f&&this.delete_excessive_thread_rows();f||(b._refresh=1);d||(d=this.display_message(this.get_label("move"==a?"movingmessage":"deletingmessage"),"loading"));this.http_post(a,b,d)};this.selection_post_data=function(a){"object"!=typeof a&&(a={});a._mbox=this.env.mailbox;if(!a._uid){var b=this.env.uid?[this.env.uid]:this.message_list.get_selection();a._uid=this.uids_to_list(b)}this.env.action&&
...
e=$("input[name='_subject']"),f=$("[name='_message']").get(0),h="1"==$("input[name='_is_html']").val(),g=["cc","bcc","replyto","followupto"],k,m=this.opener();m&&"compose"==m.env.action&&(setTimeout(function(){1<opener.history.length?opener.history.back():m.redirect(m.get_task_url("mail"))},100),this.env.opened_extwin=!0);0<this.env.autocomplete_threads&&(k={threads:this.env.autocomplete_threads,sources:this.env.autocomplete_sources});this.init_address_input_events(d,k);for(a in g)this.init_address_input_events($("[name='_"+
g[a]+"']"),k);h||(a=this.env.top_posting?0:f.value.length,this.set_caret_pos(f,a),"select-one"==b.prop("type")&&this.change_identity(b[0]),a&&$(f).scrollTop(f.scrollHeight));this.env.save_localstorage&&this.compose_restore_dialog(0,h);""==d.val()?d.focus():""==e.val()?e.focus():f&&f.focus();this.env.compose_focus_elem=document.activeElement;this.compose_field_hash(!0);this.auto_save_start()};this.compose_restore_dialog=function(a,b){var d,e,f,h=this.local_storage_get_item("compose.index",[]),n=function(a){++a<
h.length&&g.compose_restore_dialog(a,b)};for(d=a||0;d<h.length;d++)if(e=h[d],f=this.local_storage_get_item("compose."+e,null,!0)){if(f.changed&&e==this.env.compose_id){this.restore_compose_form(e,b);break}if(!(this.env.draft_id&&f.draft_id&&f.draft_id!=this.env.draft_id||this.env.reply_msgid&&f.reply_msgid!=this.env.reply_msgid)&&f.changed&&f.session!=this.env.session_id){this.show_popup_dialog(this.get_label("restoresavedcomposedata").replace("$date",(new Date(f.changed)).toLocaleString()).replace("$subject",
...
this.env.recipients_separator+" ";a.keydown(function(a){return g.ksearch_keydown(a,this,b)}).attr({autocomplete:"off","aria-autocomplete":"list","aria-expanded":"false",role:"combobox"})};this.submit_messageform=function(a){var b=this.gui_objects.messageform;if(b){var d=this.set_busy(!0,a?"savingmessage":"sendingmessage"),e=this.spellcheck_lang(),f=[];$("li",this.gui_objects.attachmentlist).each(function(){f.push(this.id.replace(/^rcmfile/,""))});$('input[name="_attachments"]',b).val(f.join());b.target=
"savetarget";b._draft.value=a?"1":"";b.action=this.add_url(b.action,"_unlock",d);b.action=this.add_url(b.action,"_lang",e);b.action=this.add_url(b.action,"_framed",1);this.submit_timer=setTimeout(function(){g.set_busy(!1,null,d);g.display_message(g.get_label("requesttimedout"),"error")},1E3*this.env.request_timeout);b.submit()}};this.compose_recipient_select=function(a){var b,d,e=0;for(d=0;d<a.selection.length;d++)b=a.selection[d],this.env.contactdata[b]&&e++;this.enable_command("add-recipient",e)};
this.compose_add_recipient=function(a){a||(a=$(this.env.focused_field).filter(":visible"),a=a.length?a.attr("id").replace("_",""):"to");var b=[],d=$("#_"+a),e=this.env.recipients_delimiter;if(this.contact_list&&this.contact_list.selection.length)for(var f,h=0;h<this.contact_list.selection.length;h++)if((f=this.contact_list.selection[h])&&this.env.contactdata[f]&&(b.push(this.env.contactdata[f]),"E"==f.charAt(0)&&0>this.env.contactdata[f].indexOf("@")&&d.length)){var g=f.substr(1);this.group2expand[g]=
{name:this.env.contactdata[f],input:d.get(0)};this.http_request("group-expand",{_source:this.env.source,_gid:g},!1)}b.length&&d.length&&(f=d.val(),h=new RegExp(RegExp.escape(e)+"\\s*$"),f&&!h.test(f)&&(f+=e+" "),d.val(f+b.join(e+" ")+e+" "),this.triggerEvent("add-recipient",{field:a,recipients:b}));return b.length};this.check_compose_input=function(a){var b=$("[name='_to']"),d=$("[name='_cc']"),e=$("[name='_bcc']"),f=$("[name='_from']"),h=$("[name='_subject']");if("text"==f.prop("type")&&!rcube_check_email(f.val(),
...
this.enable_command("insert-sig",!1);if(!this.env.identities_initialized&&(this.env.identities_initialized=!0,this.env.show_sig_later&&(this.env.show_sig=!0),this.env.opened_extwin))return;$.each(["replyto","bcc"],function(){var a,b=e&&g.env.identities[e]?g.env.identities[e][this]:"",m=d&&g.env.identities[d]?g.env.identities[d][this]:"",l=$('[name="_'+this+'"]'),p=l.val();b&&p&&(a=new RegExp("\\s*"+RegExp.escape(b)+"\\s*"),p=p.replace(a,""));a=new RegExp(h+"\\s*"+h,"g");p=String(p).replace(a,f);a=
new RegExp("^[\\s"+h+"]+");p=p.replace(a,"");m&&-1==p.indexOf(m)&&-1==p.indexOf(m.replace(/"/g,""))&&(p&&(a=new RegExp("["+h+"\\s]+$"),p=p.replace(a,"")+f+" "),p+=m+f+" ");(b||m)&&l.val(p).change()});this.editor.change_signature(d,b);this.env.identity=d;this.triggerEvent("change_identity");return!0};this.upload_file=function(a,b){if(a){var d=0,e=0;$("input[type=file]",a).each(function(a,b){var f=b.files?b.files.length:b.value?1:0;if(b.files)for(a=0;a<f;a++)d+=b.files[a].size;e+=f});if(e){if(this.env.max_filesize&&
this.env.filesizeerror&&d>this.env.max_filesize)return this.display_message(this.env.filesizeerror,"error"),!1;var f=this.async_upload_form(a,b||"upload",function(a){var b,d="";try{this.contentDocument?b=this.contentDocument:this.contentWindow&&(b=this.contentWindow.document),d=b.childNodes[1].innerHTML}catch(e){}d.match(/add2attachment/)||bw.opera&&(!g.env.uploadframe||g.env.uploadframe!=a.data.ts)||(d.match(/display_message/)||g.display_message(g.get_label("fileuploaderror"),"error"),g.remove_from_attachment_list(a.data.ts));
...
this.upload_progress_start(a.action,a.name))};this.add_contact=function(a){a&&this.http_post("addcontact",{_address:a});return!0};this.qsearch=function(a){if(""!=a){var b=this.set_busy(!0,"searching");a=this.search_params(a);var d="compose"==this.env.action&&this.contact_list?"search-contacts":"search";this.message_list?this.clear_message_list():this.contact_list&&this.list_contacts_clear();this.env.source&&(a._source=this.env.source);this.env.group&&(a._gid=this.env.group);this.env.current_page=
1;a=this.http_request(d,a,b);this.env.qsearch={lock:b,request:a};this.enable_command("set-listmode",this.env.threads&&"base"==(this.env.search_scope||"base"));return!0}return!1};this.continue_search=function(a){var b=this.set_busy(!0,"stillsearching");setTimeout(function(){var d=g.search_params();d._continue=a;g.env.qsearch={lock:b,request:g.http_request("search",d,b)}},100)};this.search_params=function(a,b){var d,e={},f=[],h=this.env.search_mods,g=this.env.search_scope||"base",k="all"==g?"*":this.env.mailbox;
!b&&this.gui_objects.search_filter&&(b=this.gui_objects.search_filter.value);!a&&this.gui_objects.qsearchbox&&(a=this.gui_objects.qsearchbox.value);b&&(e._filter=b);if(a&&(e._q=a,h&&this.message_list&&(h=h[k]||h["*"]),h)){for(d in h)f.push(d);e._headers=f.join(",")}g&&(e._scope=g);k&&"all"!=g&&(e._mbox=k);return e};this.reset_qsearch=function(){this.gui_objects.qsearchbox&&(this.gui_objects.qsearchbox.value="");this.env.qsearch&&this.abort_request(this.env.qsearch);this.env.qsearch=null;this.env.search_request=
null;this.env.search_id=null;this.enable_command("set-listmode",this.env.threads)};this.set_searchscope=function(a){var b=this.env.search_scope;this.env.search_scope=a;a!=b&&this.env.search_request&&(!this.qsearch(this.gui_objects.qsearchbox.value)&&this.env.search_filter&&"ALL"!=this.env.search_filter&&this.filter_mailbox(this.env.search_filter),"all"!=a&&this.select_folder(this.env.mailbox,"",!0))};this.set_searchmods=function(a){var b=this.env.mailbox;"all"==(this.env.search_scope||"base")&&(b=
"*");this.env.search_mods||(this.env.search_mods={});b&&(this.env.search_mods[b]=a)};this.is_multifolder_listing=function(){return void 0!==this.env.multifolder_listing?this.env.multifolder_listing:this.env.search_request&&"base"!=(this.env.search_scope||"base")};this.sent_successfully=function(a,b,d){this.display_message(b,a);this.compose_skip_unsavedcheck=!0;if(this.env.extwin){this.lock_form(this.gui_objects.messageform);var e={task:"mail",action:""};if(e=this.opener(!1,e)||this.opener(!0,e))e.display_message(b,
a),d&&0<=$.inArray(e.env.mailbox,d)&&e.command("checkmail");setTimeout(function(){window.close()},1E3)}else setTimeout(function(){g.list_mailbox()},500)};this.ksearch_keydown=function(a,b,d){this.ksearch_timer&&clearTimeout(this.ksearch_timer);var e=rcube_event.get_keycode(a),f=rcube_event.get_modifier(a);switch(e){case 38:case 40:if(!this.ksearch_visible())return;b=38==e?1:0;e=document.getElementById("rcmkSearchItem"+this.ksearch_selected);e||(e=this.ksearch_pane.__ul.firstChild);e&&this.ksearch_select(b?
e.previousSibling:e.nextSibling);return rcube_event.cancel(a);case 9:if(f==SHIFT_KEY||!this.ksearch_visible()){this.ksearch_hide();return}case 13:if(!this.ksearch_visible())return!1;this.insert_recipient(this.ksearch_selected);this.ksearch_hide();return rcube_event.cancel(a);case 27:this.ksearch_hide();return;case 37:case 39:return}this.ksearch_timer=setTimeout(function(){g.ksearch_get_results(d)},200);this.ksearch_input=b;return!0};this.ksearch_visible=function(){return null!==this.ksearch_selected&&
void 0!==this.ksearch_selected&&this.ksearch_value};this.ksearch_select=function(a){this.ksearch_pane&&a&&this.ksearch_pane.find("li.selected").removeClass("selected").removeAttr("aria-selected");a&&($(a).addClass("selected").attr("aria-selected","true"),this.ksearch_selected=a._rcm_id,$(this.ksearch_input).attr("aria-activedescendant","rcmkSearchItem"+this.ksearch_selected))};this.insert_recipient=function(a){if(null!==a&&this.env.contacts[a]&&this.ksearch_input){var b=this.ksearch_input.value,d=
...
this.ksearch_msg&&this.hide_message(this.ksearch_msg);this.ksearch_msg=this.ksearch_info=this.ksearch_data=null};this.contactlist_keypress=function(a){a.key_pressed==a.DELETE_KEY&&this.command("delete")};this.contactlist_select=function(a){this.preview_timer&&clearTimeout(this.preview_timer);var b,d,e,f,h=!1,n=this.env.source?this.env.address_sources[this.env.source]:null;this.env.contentframe&&(d=a.get_single_selection())?this.preview_timer=setTimeout(function(){g.load_contact(d,"show")},200):this.env.contentframe&&
this.show_contentframe(!1);if(a.selection.length){a.draggable=!1;this.env.selection_sources=[];n&&this.env.selection_sources.push(this.env.source);for(b in a.selection)f=a.data[a.selection[b]],n?h=h||!n.readonly&&!f.readonly:(e=String(a.selection[b]).replace(/^[^-]+-/,""))&&this.env.address_sources[e]&&(h=h||!this.env.address_sources[e].readonly&&!f.readonly,this.env.selection_sources.push(e)),"group"!=f._type&&(a.draggable=!0);this.env.selection_sources=$.unique(this.env.selection_sources)}this.enable_command("group-remove-selected",
this.env.group&&0<a.selection.length&&h);this.enable_command("compose",this.env.group||0<a.selection.length);this.enable_command("export-selected","copy",0<a.selection.length);this.enable_command("edit",d&&h);this.enable_command("delete","move",0<a.selection.length&&h);return!1};this.list_contacts=function(a,b,d){var e,f={},h=void 0===a&&void 0===b&&void 0===d,g=window;a||(a=this.env.source);h&&(b=this.env.group);if(d&&this.current_page==d&&a==this.env.source&&b==this.env.group)return!1;a!=this.env.source?
(d=this.env.current_page=1,this.reset_qsearch()):h||b==this.env.group||(d=this.env.current_page=1);this.env.search_id?e="S"+this.env.search_id:this.env.search_request||(e=b?"G"+a+b:a);this.env.source=a;this.env.group=b;h=$.inArray(this.env.group,this.env.address_group_stack);this.env.address_group_stack=0>h?[]:this.env.address_group_stack.slice(0,h);this.env.group?(this.env.address_group_stack.push(this.env.group),e="G"+a+this.env.address_group_stack[0]):this.gui_objects.addresslist_title&&$(this.gui_objects.addresslist_title).html(this.get_label("contacts"));
...
d.length;e++)id=d[e],f.push(id),this.contact_list.remove_row(id,e==d.length-1);1==d.length&&this.show_contentframe(!1)}b||(b={});b._source=this.env.source;b._from=this.env.action;b._cid=f.join(",");this.env.group&&(b._gid=this.env.group);this.env.search_request&&(b._search=this.env.search_request);this.http_post(a,b,h);return!0}};this.update_contact_row=function(a,b,d,e,f){var h=this.contact_list;a=this.html_identifier(a);h.rows[a]||(a=a+"-"+e,d&&(d=d+"-"+e));h.update_row(a,b,d,!0);h.data[a]=f};this.add_contact_row=
function(a,b,d,e){if(!this.gui_objects.contactslist)return!1;var f,h=this.contact_list,g={cols:[]};g.id="rcmrow"+this.html_identifier(a);g.className="contact "+(d||"");h.in_selection(a)&&(g.className+=" selected");for(f in b)d={},d.className=String(f).toLowerCase(),d.innerHTML=b[f],g.cols.push(d);h.data[a]=e;h.insert_row(g);this.enable_command("export",0<h.rowcount)};this.init_contact_form=function(){var a;if(this.env.coltypes)for(a in this.set_photo_actions($("#ff_photo").val()),this.env.coltypes)this.init_edit_field(a,
...
b).last(),e=$("<fieldset>").addClass("contactfieldgroup contactcontroller"+a);f.length?e.insertAfter(f):b.prepend(e)}if(e.length&&"FIELDSET"==e.get(0).nodeName){var h;b=this.env.coltypes[a];var n="ff_"+a+(b.count||0),f=$("<div>").addClass("row"),k=$("<div>").addClass("contactfieldcontent data"),m=$("<div>").addClass("contactfieldlabel label");b.subtypes_select?m.html(b.subtypes_select):m.html('<label for="'+n+'">'+b.label+"</label>");var l=1!=b.limit?"[]":"";if("text"==b.type||"date"==b.type)h=$("<input>").addClass("ff_"+
a).attr({type:"text",name:"_"+a+l,size:b.size,id:n}).appendTo(k),this.init_edit_field(a,h),"date"==b.type&&$.datepicker&&h.datepicker();else if("textarea"==b.type)h=$("<textarea>").addClass("ff_"+a).attr({name:"_"+a+l,cols:b.size,rows:b.rows,id:n}).appendTo(k),this.init_edit_field(a,h);else if("composite"==b.type){var p,q,w=[],t=[];if(h=this.env[a+"_template"])for(n=0;n<h.length;n++)w.push(h[n][1]),t.push(h[n][2]);else for(p in b.childs)w.push(p);for(n=0;n<w.length;n++)p=w[n],h=b.childs[p],h=$("<input>").addClass("ff_"+
...
function(a,b){var d=this.folder_selector_element;if(!d){var e=[],f=this.env.delimiter,h=$('<ul class="toolbarmenu">'),n=document.createElement("a"),d=$('<div id="folder-selector" class="popupmenu"></div>');n.href="#";n.className="icon";$.each(this.env.mailboxes_list,function(){var a=0,b=0,d=g.env.mailboxes[this],h=d.id,q=$(n.cloneNode(!1)),w=$("<li>");d.virtual?q.addClass("virtual").attr("aria-disabled","true").attr("tabindex","-1"):q.addClass("active").data("id",d.id);for(d["class"]&&q.addClass(d["class"]);0<=
(b=h.indexOf(f,b));)a++,b++;q.css("padding-left",a?16*a+"px":0);q.append($("<span>").text(d.name));w.append(q);e.push(w)});h.append(e).appendTo(d);d.css({left:"-1000px",top:"-1000px"}).appendTo($("body")).show();10<e.length&&d.css("max-height",10*$("li",d)[0].offsetHeight+9);d.on("click","a.active",function(a){d.data("callback")($(this).data("id"));return!1});this.folder_selector_element=d}d.data("callback",b);this.show_menu("folder-selector",!0,a)};this.show_menu=function(a,b,d){var e="object"==
typeof a?a.menu:a,f=$("#"+e),g=d&&d.target?$(d.target):$(f.attr("rel")||"#"+e+"link"),n=rcube_event.is_keyboard(d),k=f.attr("data-align")||"",m=!1;"A"!=g.get(0).tagName&&g.closest("a").length&&(g=g.closest("a"));"string"==typeof a&&(a={menu:e});f.length||(f=this.triggerEvent("menu-get",{name:e,props:a,originalEvent:d}));if(!f||!f.length)return this.triggerEvent(!1===b?"menu-close":"menu-open",{name:e,props:a,originalEvent:d});f.appendTo(document.body);"undefined"==typeof b&&(b=f.is(":visible")?!1:
!0);if(b&&g.length){var l=$(window),p=g.offset(),q=0<=k.indexOf("bottom"),m="menuitem"==g.attr("role")||0<g.closest("[role=menuitem]").length;g.offsetWidth=g.outerWidth();g.offsetHeight=g.outerHeight();!q&&p.top+g.offsetHeight+f.height()>l.height()&&(q=!0);0<=k.indexOf("right")?p.left=p.left+g.outerWidth()-f.width():m&&(p.left=p.left+g.offsetWidth-5,p.top-=g.offsetHeight);p.left+f.width()>l.width()&&(p.left=l.width()-f.width()-12);p.top=Math.max(0,p.top+(q?-f.height():g.offsetHeight));f.css({left:p.left+
"px",top:p.top+"px"})}if(b){for(k=this.menu_stack.length-1;m&&0<=k;k--)$(g).parents("#"+this.menu_stack[k]).length||this.hide_menu(this.menu_stack[k],d);m&&this.menu_stack.length?(f.data("parent",$.last(this.menu_stack)),f.css("z-index",($("#"+$.last(this.menu_stack)).css("z-index")||0)+1)):!m&&this.menu_stack.length&&this.hide_menu(this.menu_stack[0],d);f.show().attr("aria-hidden","false").data("opener",g.attr("aria-expanded","true").get(0));this.triggerEvent("menu-open",{name:e,obj:f,props:a,originalEvent:d});
this.menu_stack.push(e);if(this.menu_keyboard_active=b&&n)this.focused_menu=e,f.find("a,input:not(:disabled)").not("[aria-disabled=true]").first().focus()}else this.hide_menu(e,d);return b};this.hide_menu=function(a,b){if(this.menu_stack.length){for(var d,e=rcube_event.is_keyboard(b),f=this.menu_stack.length-1;0<=f;f--)d=$("#"+this.menu_stack[f]).hide().attr("aria-hidden","true").data("parent",!1),this.triggerEvent("menu-close",{name:this.menu_stack[f],obj:d,props:{menu:this.menu_stack[f]},originalEvent:b}),
this.menu_stack[f]==a&&(f=-1,d.data("opener")&&($(d.data("opener")).attr("aria-expanded","false"),e&&d.data("opener").focus())),this.menu_stack.pop();this.menu_stack.length&&e?(this.menu_keyboard_active=!0,this.focused_menu=$.last(this.menu_stack),d&&d.data("opener")||$("#"+this.focused_menu).find("a,input:not(:disabled)").not("[aria-disabled=true]").first().focus()):(this.focused_menu=null,this.menu_keyboard_active=!1)}else this.triggerEvent("menu-close",{name:a,props:{menu:a},originalEvent:b})};
this.element_position=function(a,b){b=$(b);var d=$(window),e=b.outerWidth(),f=b.outerHeight(),g=b.data("menu-pos"),n=d.height(),k=$(a).height(),m=$(a).width(),l=b.offset(),p=l.top,l=l.left+e;"bottom"==g?(p+=f,l-=e):l-=5;p+k>n&&(p-=k-f,0>p&&(p=Math.max(0,(n-k)/2)));l+m>d.width()&&(l-=m+e);a.css({left:l+"px",top:p+"px"})};this.editor_init=function(a,b){this.editor=new rcube_text_editor(a,b)};this.html2plain=function(a,b){return this.format_converter(a,"html",b)};this.plain2html=function(a,b){return this.format_converter(a,
...
return this.local_storage_prefix};this.local_storage_get_item=function(a,b,d){var e,f;try{e=localStorage.getItem(this.get_local_storage_prefix()+a),f=JSON.parse(e)}catch(g){}return f||b||null};this.local_storage_set_item=function(a,b,d){try{return localStorage.setItem(this.get_local_storage_prefix()+a,JSON.stringify(b)),!0}catch(e){return!1}};this.local_storage_remove_item=function(a){try{return localStorage.removeItem(this.get_local_storage_prefix()+a),!0}catch(b){return!1}}}
rcube_webmail.long_subject_title=function(g,v){if(!g.title){var a=$(g);a.width()+15*(v||0)>a.parent().width()&&(g.title=rcube_webmail.subject_text(g))}};rcube_webmail.long_subject_title_ex=function(g){if(!g.title){var v=$(g),a=$.trim(v.text()),a=$("<span>").text(a).css({position:"absolute","float":"left",visibility:"hidden","font-size":v.css("font-size"),"font-weight":v.css("font-weight")}).appendTo($("body")),b=a.width();a.remove();b+15*$("span.branch",v).width()>v.width()&&(g.title=rcube_webmail.subject_text(g))}};
rcube_webmail.subject_text=function(g){g=$(g).clone();g.find(".skip-on-drag").remove();return g.text()};rcube_webmail.prototype.get_cookie=getCookie;rcube_webmail.prototype.addEventListener=rcube_event_engine.prototype.addEventListener;rcube_webmail.prototype.removeEventListener=rcube_event_engine.prototype.removeEventListener;rcube_webmail.prototype.triggerEvent=rcube_event_engine.prototype.triggerEvent;