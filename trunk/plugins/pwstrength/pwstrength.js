        ...
        methods = {
            init : function( options, callbacks) {
                ...
                settings = $.extend({
                    ...
                    'passwordValidFrom': rcmail.env.pwstrength_required, // 50%
...
function pwstrength_sync_background(){
  var password = $("[name='" + rcmail.env.pwstrength_fieldname + "']");
  var password_confirm = $("[name='" + rcmail.env.pwstrength_fieldname_confirm + "']");
  if(password.val() != '' && password.val() == password_confirm.val()){
    $("input[name='" + rcmail.env.pwstrength_fieldname_confirm + "']").attr('style', $("input[name='" + rcmail.env.pwstrength_fieldname + "']").attr('style'));
  }
  else{
    $("input[name='" + rcmail.env.pwstrength_fieldname_confirm + "']").attr('style', '');
  }
  window.setTimeout('pwstrength_sync_background()', 200);
}

$(document).ready(function(){
    pwstrength_sync_background();
    $("input[name='" + rcmail.env.pwstrength_fieldname + "']").pStrength({
        ...
        'onPasswordStrengthChanged' : function(passwordStrength, strengthPercentage) {
            ...
            $('#' + $(this).data('display')).html(rcmail.gettext('pwstrength.pwstrength').replace('%s', strengthPercentage));
            $("input[name='_pwstrength']").val(0);
        },
        'onValidatePassword': function(strengthPercentage) {
            rcmail.display_message(rcmail.gettext('pwstrength.continue'), 'confirmation');
