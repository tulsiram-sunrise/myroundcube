...
class carddav_backend
{
	...
	*/
	...
	public function get_ctag($raw = false)
	{
	  ...
	  if ($response === false || $raw === true)
		...
		{
			preg_match_all('/getctag(.*?)>(.*?)<\/(.*?)getctag>/i', $response, $matches);
  ...
  */
  public function autodiscover_principal()
  {
    ...
    if(isset($xml->response->propstat->prop->{'current-user-principal'}->href[0])){
      $base_url = str_replace($xml->response->href, '', $this->url);
      return $base_url . $xml->response->propstat->prop->{'current-user-principal'}->href[0];
    }
    else{
      return false;
    }
  ...
  */
  public function autodiscover_addressbooks_home()
  {
    ...
    if(isset($xml->response->propstat->prop->{'current-user-principal'}->href[0])){
      $base_url = str_replace($xml->response->href, '', $this->url);
      $this->url = $base_url . $xml->response->propstat->prop->{'current-user-principal'}->href[0];
      $this->headers = array('depth' => 'Depth: 1');
      ...
      if(isset($xml->response->propstat->prop->{'caraddressbook-home-set'}->href[0])){
        return $base_url . $xml->response->propstat->prop->{'caraddressbook-home-set'}->href[0];
      }
      else{
        return false;
      }
  ...
  */
  public function autodiscover_addressbooks()
  {
    ...
    if(isset($xml->response->propstat->prop->{'current-user-principal'}->href[0])){
      ...
      if(isset($xml->response->propstat->prop->{'caraddressbook-home-set'}->href[0])){
        $this->url = $base_url . $xml->response->propstat->prop->{'caraddressbook-home-set'}->href[0];
        $content = '<?xml version="1.0"?><d:propfind xmlns:d="DAV:"><d:prop><d:resourcetype /><d:displayname /></d:prop></d:propfind>';
        $content_type = 'application/xml';
        $response = $this->clean_response($this->query($this->url, 'PROPFIND', $content, $content_type));
        try{
          $xml = new SimpleXMLElement($response);
        }
        catch (Exception $e){
        ...
        if(is_object($xml->response)){
          foreach($xml->response as $addressbook){
            if(urlencode(urldecode($base_url . $addressbook->href)) != urlencode(urldecode($this->url))){
              $addressbooks[] = array(
                'href' => $base_url . $addressbook->href,
                'name' => (string) $addressbook->propstat->prop->displayname[0],
              );
            }
          }
        }
