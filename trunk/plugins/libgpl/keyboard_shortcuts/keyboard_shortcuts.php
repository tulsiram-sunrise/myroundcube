...
class keyboard_shortcuts_core extends rcube_plugin
{
    ...
    function init()
    {
        ...
        // include js
        if($this->rcmail->output->type == 'html'){
          $this->rcmail->output->add_header(html::tag('script', array('type' => 'text/javascript', 'src' => 'plugins/libgpl/keyboard_shortcuts/keyboard_shortcuts.js')));
          $this->rcmail->output->add_header(html::tag('script', array('type' => 'text/javascript', 'src' => 'plugins/libgpl/keyboard_shortcuts/shortcuts.js')));
        }
    ...
    private function get_shortcuts()
    {
        ...
        // unset tabbed shortcuts if tabbed plugin has been disabled
        foreach($shortcuts as $section => $props){
          foreach($shortcuts[$section] as $key => $command){
            if(strpos($command, 'tabbed') !== false){
        ...
        if($args['section'] == 'keyboard_shortcuts') {
            ...
            foreach($commands as $action => $a) {
                ...
                $args['blocks'][$action]['options']['add'] = array(
                    'title' => '',
                    'content' => html::tag('span', array('class' => 'ks_content'), html::tag('a', array('class' => 'button ks_add myrc_sprites', 'data-section' => $action), ''))
                );
            }
    ...
    private function create_row($section, $key = false)
    {
        ...
        // del button
        $button = html::a(array('class' => 'button ks_del myrc_sprites'), '');
        ...
        if ($p['name'] == "listcontrols") {
            ...
            $content = html::tag('a', array('class' => 'keyboard_shortcuts_icon',
                                ...
                                html::tag('img', array( 'src' => $icon,
                                                        'class' => 'myrc_sprites keyboard_shortcuts_icon',
                                                        'align' => 'top',
