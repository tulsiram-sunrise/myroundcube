  ...
  */
  ...
  private function carddav_addressbook_add($carddav_content){
    ...
    if(is_array($exists)){
      ...
      $query = "INSERT INTO " . get_table_name($this->db_name) . 
               ...
               " VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
      $rcmail->db->set_option('ignore_key_errors', true);
      ...
      if($rcmail->db->affected_rows($result)){
        ...
        if($rcmail->action == 'refresh' || $rcmail->action == 'plugin.carddav-server-save'){
    ...
    if(isset($save_data['groups']) && is_array($save_data['groups'])){
      ...
      foreach($categories as $category){
        ...
        if(!is_array($exists)){
          ...
          $rcmail->db->query($sql, $rcmail->user->ID, date('Y-m-d H:i:s'), 0, $category, $this->carddav_server_id);
          $contactgroup_id = $rcmail->db->insert_id(get_table_name($this->db_groups));
        }
        else{
  ...
  private function carddav_add($vcard){
    ...
    if($carddav_backend->check_connection()){
      ...
      if($sync && $rcmail->action != 'import'){
        if($rcmail->action == 'copy'){
          $vcard_id = false;
        }
        $this->carddav_addressbook_sync($server, false, $vcard_id);
        $cid = $rcmail->db->insert_id(get_table_name($this->db_name));
        return $cid;
      }
      else{
        return true;
