...
define('CALEOT', '2037-12-31 23:59:59');
...
class calendar extends rcube_plugin{
  ...
  static private $authors_comments = '<a href="http://myroundcube.com/myroundcube-plugins/calendar-plugin" target="_blank">Documentation</a><br /><a href="http://trac.roundcube.net/ticket/1489912" target="_blank">Related Ticket</a><br /><font color="red">IMPORTANT</font>: Please <a href="http://mirror.myroundcube.com/docs/session.html" target="_blank">patch</a><i> ./program/include/rcmail.php</i> and update config and add <i>$config["default_caldavs"] = isset($config["caldavs"]) ? $config["caldavs"] : array();</i> at bottom of your configuration.';
  static private $version = '18.1.31';
  static private $date = '09-07-2014';
  ...
  static private $requirements = array(
    'extra' => '<span style="color: #ff0000;">IMPORTANT</span> &#8211; <a href="http://mirror.myroundcube.com/docs/session.html" target="_blank">Apply Core Patches</a>',
    'Roundcube' => '1.0.1',
    'PHP' => '5.3',
  ...
  *****************************/
  ...
  function init() {
      ...
      if(!get_input_value('_remote', RCUBE_INPUT_GPC)){
        ...
        if($t[0] >= '3'){
          if($rcmail->action == 'plugin.calendar'){
            if($rcmail->config->get('caldav_tasks', true)){
              $rcmail->output->add_script('$("#taskstogglecontainer").show();', 'docready');
            }
            else{
              $rcmail->output->add_script('rcmail.set_cookie("tasksvisible", 0); $("#taskstogglecontainer").hide(); $("tasks").hide();', 'docready');
            }
          }
        }
        else{
    ...
    if(!get_input_value('_framed', RCUBE_INPUT_GPC) && $rcmail->action != 'compose' && $rcmail->action != 'plugin.plugin_manager_update' && !$rcmail->config->get('cal_disable_reminders')){
  ...
  ****************************/
  ...
  function setupBackend($type = false){
    ...
    if($backend_type == "caldav"){
      ...
      if(is_string($default_caldav)){
        if(strtolower($default_caldav) == 'google'){
          $default_caldav = array();
        }
      }
      ...
      if(is_array($default_caldav) &&
        ...
        $save = array(
                        ...
                        'caldav_url' => ($default_caldav['url'] ? $default_caldav['url'] : $rcmail->config->get('caldav_url', 'https://apidata.googleusercontent.com/caldav/v2/%u/events')),
                        ...
                        'caldav_extr' => ($default_caldav['extr'] ? true : false),
                      );
      }
  ...
  ****************************/
  ...
  function generateHTML(){
    ...
    if(is_array($categories)){
      $default_url = strtolower($rcmail->config->get('caldav_url'));
      $temp = parse_url($default_url);
      if(isset($temp['host']) || $default_url == 'google' || $default_url == ''){
        ...
        if($rcmail->config->get('backend') != 'caldav' || $rcmail->config->get('default_caldav_subscribed', true)){
          if(substr($default_url, 0, strlen('https://apidata.googleusercontent.com/caldav/v2/')) == 'https://apidata.googleusercontent.com/caldav/v2/'){
  ...
  ****************************/
  ...
  function reminders_import(){
    ...
    $res = $rcmail->db->limitquery('SELECT * FROM ' . get_table_name('users') . ' WHERE user_id=?', 0, 1, $userid);
  ...
  function reminders_mailto($p){
    ...
    if(empty($list)){
      $list = array(
        0 => array(
          'name' => $rcmail->user->data['username'],
  ...
  ****************************/
  ...
  function sendInvitation() {
    ...
    $dbtable = get_input_value('_dbtable', RCUBE_INPUT_GPC);
    if($edit == 'false'){
      $edit = 0;
    }
    else{
    ...
    $this->utils->wrap_backend($this->backend);
    $ical = $this->utils->exportEvents(0, 0, array($event), false, false, false, $event['component']);
    ...
    if(file_put_contents($file, $ical)){
      ...
      $rcmail->output->redirect(array('_task'=> 'mail', '_action' => 'compose', '_attachics' => 1, '_eid' => $id, '_dbtable' => $dbtable, '_edit' => $edit));
    }
    else{
  ...
  function message_compose($args){
    ...
    if(file_exists($ics) && file_exists($html)){
      $events_table = $this->backend->dbtable;
      $this->backend->dbtable = $args['param']['dbtable'];
      $this->utils->wrap_backend($this->backend);
  ...
  ****************************/
  ...
  function getCalDAVs($show=true){
    ...
    if(
      ...
      if(is_array($rcmail->config->get('default_caldav_backend'))){
        $properties = $rcmail->config->get('default_caldav_backend', array());
      }
      else{
        $properties = $rcmail->config->get('caldav_backend', array());
      }
  ...
  function subscribe(){
    ...
    $command = 'reload';
    if($_SESSION['user_id']){
      $rcmail->session->remove('cal_cache_events');
  ...
  function calendarLink($args){
    ...
    if(!$rcmail->config->get('cal_hide_sharing')){
  ...
  function settingsTable($args){
    ...
    if($args['section'] == 'calendarlink'){
      ...
      if(!$protected){
        ...
        $select = new html_select(array('name' => '_backend', 'id' => $field_id, 'onchange' => '$(".mainaction").hide();document.forms.form.submit()'));
        $select->add($rcmail->config->get('product_name'), 'database');
        if(strtolower($rcmail->config->get('default_caldav_backend', '')) != 'google'){
          $select->add('CalDAV', 'caldav');
        }
        if(class_exists('google_oauth2')){
          $select->add('Google', '');
        }
        if($rcmail->config->get('backend') == 'database' || (strpos($default_url, 'https://apidata.googleusercontent.com/caldav/v2/') === false && $default_url != 'google')){
      ...
      if($rcmail->config->get('backend') == 'database'){
        ...
        if(strpos($default_url, 'https://apidata.googleusercontent.com/caldav/v2/') === false && $default_url != 'google'){
          ...
          if(!$caldav_url && is_array($last_caldav) && isset($last_caldav['url'])){
            ...
            if($caldav_url){
              $rcmail->output->add_script('$(".mainaction").hide();document.forms.form.submit();', 'docready');
            }
          }
        ...
        if(!$rcmail->config->get('cal_disable_reminders')){
      ...
      foreach($a_weekday as $day => $num){
        ...
        $days .= html::tag('td', array('style' => 'border-bottom:0px;text-align:center;padding:0 2px'), $this->gettext($day));
        $controls .= html::tag('td', array('style' => 'border-bottom:0px;text-align:center;padding:0 2px'), $checkbox->show($enabled?$num:false));
      }
      $field_id = 'rcmfd_workdays';
      $table = html::tag('table', array('style' => 'width: 1px'), html::tag('tr', null, $days) . html::tag('tr', null, $controls));
      $args['blocks']['calendar']['options']['workdays'] = array(
        'title' => html::label($field_id, $this->gettext('workdays')),
        'content' => html::tag('div', array('style' => 'margin-left:-5px'), $table),
      );
      ...
      if($is_set){
        ...
        if($dsn = $rcmail->config->get('db_sabredav_dsn')){
          ...
          $db->db_connect('r');
          $tables = array('users_cal_rw', 'users_cal_r');
          foreach($tables as $table){
            $sql = 'SELECT * FROM ' . get_table_name($table) . ' WHERE username=?';
            $res = $db->limitquery($sql, 0, 1, $rcmail->user->data['username']);
        ...
        if(class_exists('sabredav') && method_exists('sabredav', 'about')){
          ...
          if($v > '3'){
            ...
            $add = true;
            foreach($identities as $key => $val){
              if(strtolower($val) == strtolower($notify)){
                $add = false;
              }
              $select->add($key, $val);
            }
            if($notify && $add){
              $select->add($notify, $notify);
            }
            $args['blocks']['calendar']['options']['caldav_notify_to'] = array(
              'title' => '- ' . html::label($field_id, Q($this->gettext('cal_notify_to'))),
              'content' => $select->show($notify),
            );
          }
        }
      }
  ...
  function saveSettings($args){
    ...
    if($args['section'] == 'calendarsharing'){
      ...
      if($_POST['_caltoken_davreadonly_submit']){
        $this->SabreDAVAuth('create', 'users_cal_r', $caltoken_davreadonly);
      }
      if($_POST['_caltoken_submit']){
        $this->SabreDAVAuth('create', 'users_cal_rw', $caltoken);
      }
      $share_rw = array();
      foreach($_POST as $key => $val){
        if(substr($key, 0, strlen('_cal_shares_')) == '_cal_shares_'){
          $share_rw[substr($key, 1)] = get_input_value($key, RCUBE_INPUT_POST);
        }
      }
      $share_r = array();
      foreach($_POST as $key => $val){
        if(substr($key, 0, strlen('_cal_shares_readonly_')) == '_cal_shares_readonly_'){
          $share_r[substr($key, 1)] = get_input_value($key, RCUBE_INPUT_POST);
        }
      }
      $this->SabreDAVShares($share_rw, $share_r);
    }
    if($args['section'] == 'calendarlink'){
      $rcmail = rcmail::get_instance();
      ...
      if(!$backend){
        $backend = 'caldav';
        $args['prefs']['caldav_url'] = $rcmail->config->get('google_caldav', 'https://apidata.googleusercontent.com/caldav/v2/' . $rcmail->config->get('google_user', '%u') . '/events');
        $args['prefs']['caldavs'] = $rcmail->config->get('caldavs', array());
        unset($args['prefs']['caldavs']['Google']);
        $args['prefs']['caldavs_subscribed'] = $rcmail->config->get('caldavs_subscribed', array());
        unset($args['prefs']['caldavs_subscribed']['Google']);
      ...
      $caldav_notify = isset($_POST['_caldav_notify']) ? true : false;
      $args['prefs']['caldav_notify'] = $caldav_notify;
      $caldav_notify_to = get_input_value('_caldav_notify_to', RCUBE_INPUT_POST);
      $args['prefs']['caldav_notify_to'] = $caldav_notify_to;
      $this->SabreDAVNotify($caldav_notify, $caldav_notify_to);
    }
  ...
  function SabreDAVNotify($enabled, $email){
    $rcmail = rcmail::get_instance();
    if($_SESSION['user_id']){
      if($dsn = $rcmail->config->get('db_sabredav_dsn')){
        $db = rcube_db::factory($dsn, '', false);
        $db->set_debug((bool)$rcmail->config->get('sql_debug'));
        $db->db_connect('r');
        $tables = array('users_cal_r', 'users_cal_rw');
        $notify = $enabled ? $email : '';
        foreach($tables as $table){
          $sql = 'UPDATE ' . get_table_name($table) . ' SET notify=?, lang=? WHERE username=?';
          $db->query($sql, $notify, $rcmail->user->data['language'], $rcmail->user->data['username']);
        }
      }
    }
  }
  
  function SabreDAVShares($share_rw, $share_r){
    $rcmail = rcmail::get_instance();
    if($_SESSION['user_id']){
      if($dsn = $rcmail->config->get('db_sabredav_dsn')){
        $db = rcube_db::factory($dsn, '', false);
        $db->set_debug((bool)$rcmail->config->get('sql_debug'));
        $db->db_connect('r');
        $sql = 'UPDATE ' . $rcmail->db->quoteIdentifier(get_table_name('calendars')) . ' SET ' . $rcmail->db->quoteIdentifier('rw') . '=?, ' . $rcmail->db->quoteIdentifier('r') . '=? WHERE ' . $rcmail->db->quoteIdentifier('principaluri') . '=?';
        $db->query($sql, 0, 0, 'principals/' . $rcmail->user->data['username']);
        foreach($share_rw as $key => $val){
          $cal = str_replace('cal_shares_', '', $key);
          $sql = 'UPDATE ' . $rcmail->db->quoteIdentifier(get_table_name('calendars')) . ' SET ' . $rcmail->db->quoteIdentifier('rw') . '=? WHERE ' . $rcmail->db->quoteIdentifier('uri') . '=? AND ' . $rcmail->db->quoteIdentifier('principaluri') . '=?';
          $db->query($sql, $val, $cal, 'principals/' . $rcmail->user->data['username']);
        }
        foreach($share_r as $key => $val){
          $cal = str_replace('cal_shares_readonly_', '', $key);
          $sql = 'UPDATE ' . $rcmail->db->quoteIdentifier(get_table_name('calendars')) . ' SET ' . $rcmail->db->quoteIdentifier('r') . '=? WHERE ' . $rcmail->db->quoteIdentifier('uri') . '=? AND ' . $rcmail->db->quoteIdentifier('principaluri') . '=?';
          $db->query($sql, $val, $cal, 'principals/' . $rcmail->user->data['username']);
        }
      }
    }
  ...
  function SabreDAVAuth($action, $table){
    ...
    if($_SESSION['user_id']){
      ...
      $db = rcube_db::factory($sabredb, '', false);
      $db->set_debug((bool)$rcmail->config->get('sql_debug'));
      $db->db_connect('r');
      if($action == 'delete'){
        ...
        $sql = 'INSERT INTO ' . $rcmail->db->quoteIdentifier(get_table_name($table)) . ' (' . $rcmail->db->quoteIdentifier('username') . ', ' . $rcmail->db->quoteIdentifier('digesta1') . ',' . $rcmail->db->quoteIdentifier('token') . ') VALUES (?, ?, ?)';
        $digest = md5($rcmail->user->data['username'] . ':' . $rcmail->config->get('sabredav_realm') . ':' . $pass);
        $db->query($sql, $rcmail->user->data['username'], $digest, $pass);
      }
    }
    return $pass;
  }
...
<channel>
  ...
  ****************************/
  ...
  static public function getUser($id=null) {
    ...
    if(empty($id))
      return array();
    $sql_result = $rcmail->db->limitquery("SELECT * FROM ".get_table_name('users')." WHERE user_id=?", 0, 1, $id);
  ...
  function deprecate($deprecate = array()) {
    ...
    if($rcmail->user->ID && !$_SESSION['preferences_deprecate']){
      ...
      $sql = 'SELECT preferences FROM ' . get_table_name('users') . ' WHERE user_id=? AND mail_host=?';
      $result = $rcmail->db->limitquery($sql, 0, 1, $rcmail->user->ID, $rcmail->user->data['mail_host']);
