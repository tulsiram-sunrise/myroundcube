...
class calendar_ui
{
  ...
  public function init()
  {
    ...
    if($this->rc->output->type == 'html'){
      $this->rc->output->add_header(html::tag('script', array('type' => 'text/javascript', 'src' => 'plugins/libgpl/calendar/calendar_base.js')));
    }
  ...
  {
    if(strpos($this->rc->data['username'], '@') !== false){
      $domain = end(explode('@', $this->rc->data['username']));
    }
    else{
      $domain = 'domain.com';
    }
    $input = new html_inputfield(array('name' => 'participant', 'id' => 'edit-attendee-name', 'size' => 40, 'placeholder' => sprintf($this->cal->gettext('attendeeplaceholder'), $domain)));
