...
/*
                    ...
                    if($result && $task = $this->rc->db->fetch_assoc($result))
                    {
                        $attachments = array();
...
*/
        ...
        if($callback && $this->rc->action == 'refresh')
        {
            ...
            $this->events->perform_updates($updates, false);
        }
     ...
     * @return array
     */
    private function _load_all_tasks($cal_id)
    {
        $result = $this->rc->db->query(
    ...
    private function _sync_calendar($cal_id)
    {
        ...
        foreach($this->_load_all_tasks($cal_id) as $task)
        {
        ...
        if($updates)
        {
            ...
            foreach($tasks as $task)
            {
                    ...
                    array_search($task["task_id"], $synced_task_ids) === false) // No in-sync task
                {
                    ...
                    self::debug_log("Remove task \"".$task["taskid"]."\".");
                }
            }
